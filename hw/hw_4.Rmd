...
```{r}
year <- seq(from = 0, to = 30, by =0.5) #all the periods in the lifetime of the bond

payment <- rep(CPN, length(year)) #all coupon payments. first and last are incorrect
payment[length(payment)] <- CPN+1000 # last payment
payment[1] <- 0 #first payment

data.frame(year, payment) |>
  ggplot()+
  geom_col (aes(x = year, y = payment))
```
